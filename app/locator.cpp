...
Locator::Locator(){
  /// rotation matrix is conformed of [r1 r2 translation]. r3 isn't necessary because it is a redundant cross product of the first 2 columns
  _rotationM = (cv::Mat_< float > (3,3) << 0.07579, -0.02684, -0.78892, 0.0, 0.15731,-0.61428, 0,0,1);
  _transVec = (cv::Mat_< float > (1,3) << 0, 0, 1);
  _intrinsicM = (cv::Mat_< float > (3,3) << 633.319, 0.0, 494.641, 0.0, 635.66, 390.479, 0.0, 0.0, 1.0);
}
...
void Locator::checkMatrixSize(cv::Mat matrix, cv::Size size){
  if ( matrix.size().width != size.width && matrix.size().height != size.height){
    throw std::invalid_argument("Invalid Shape of Vector (Matrix shape (3x3) and Vector shape is (1x3)");
  }
}
...
void Locator::pixelVector() {
  /// Uses the public attribute of cv::Rec object to obtain width and height of bounding box.
  int midX = pixelPoint.width / 2;
  int midY = pixelPoint.height;

  /// Gets pixel coordinate and converts to a 1x3 vector.
  pixelCoord = (cv::Mat_<float>(1,3) << pixelPoint.x + midX,  pixelPoint.y + midY, 1);
}
...
void Locator::printPositions() {
  ...
  std::cout << "Position of Object in meters [x,y,z]: " << worldCoord << std::endl;
}
